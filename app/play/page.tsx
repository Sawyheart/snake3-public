...
const SNAKE_COLORS = ["black", "red", "green", "blue"] as const
...
export default function GamePage() {
  ...
  const {canvasRef, snakeGameData: { snake }, gameStatus, startGame, restartGame, stopGame} = useGame({
    startingStep: {x: Math.round(1 * Math.random() * 49), y: 0, color: SNAKE_COLORS[selectedColorIndex]},
    speed: 100,
    maxUnitForSide: 50,
    direction: [0, 1],
    snake: []
    ...
    <section className={cn("w-full min-h-[100dvh] px-10 pb-28 pt-40 flex flex-col items-center justify-center gap-10")}>
      ...
      <div className="w-[min(500px,_100%)] flex justify-between items-center gap-8">
        ...
        <section className="group round_button min-w-[64px]">
          <div className="w-full h-full flex rounded-full bg-[url('../public/assets/moving_empty_canvas.svg')] bg-right bg-[length:160px] hover:bg-[length:280px] default_transition" onClick={() => setSelectedColorIndex((idx) => (idx + 1) % SNAKE_COLORS.length )}>
            <svg className={cn("default_transition w-1/2 group-hover:w-3/4")} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10">
              <path d="M0,5 H10" stroke={SNAKE_COLORS[selectedColorIndex]} strokeWidth={4} shapeRendering="crispEdges" />
            </svg>
          </div>
        </section>
      </div>
